...
window.__ModuleLoader__.load({ id: 'dsh-custom-system-prompt', factory: (require) => {
  ...
  const COLLAPSED_KEY = 'dsh-custom-system-prompt.collapsed'
  ...
  function SettingsCard() {
    ...
    const load = (active = true) => {
      return Promise.all([readJson(SESSIONS_URL), readJson(CONFIG_URL)])
        .then(([sessionsRes, configRes]) => {
          ...
          setSessions(list)
          const current = active && list.some((s) => s.id === selected) ? selected : (list[0]?.id ?? '')
          setSelected(current)
          setText(map[current] ?? '')
    ...
    return React.createElement('li', { style: cardStyle, 'data-testid': 'dsh-custom-system-prompt-settings' },
      React.createElement('div', {
        ...
        onClick: toggleCollapsed, role: 'button', 'aria-expanded': !collapsed,
        title: collapsed ? '展开设置' : '折叠设置',
      },
        React.createElement('span', { style: { width: 14, textAlign: 'center', opacity: 0.7, fontSize: 12 } }, collapsed ? '▸' : '▾'),
        React.createElement('strong', { style: { fontSize: 16 } }, '自定义系统提示'),
      ),
      !collapsed && React.createElement('p', { style: { margin: '4px 0 0', opacity: 0.72 } }, '按会话注入系统提示：选择会话、写入文本、保存，该会话下一轮模型调用即带上。重启进程后设置保留。'),
      !collapsed && (status === 'unavailable'
        ...
        : React.createElement(React.Fragment, null,
          Field({ label: '目标会话', hint: '当前打开的会话列表（显示会话标题，新会话需刷新后出现）。',
            ...
            sessions.length === 0
              ...
              : sessions.map((s) => React.createElement('option', { key: s.id, value: s.id }, s.title ?? s.id))),
          }),
