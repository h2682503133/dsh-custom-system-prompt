 ...
 *       GET  /plugins/dsh-custom-system-prompt/sessions  — live session ids
 *
...
export const inject = ['settings', 'systemPrompt', 'agents', 'webServer']
...
export function apply(ctx) {
  ...
  const handleSessions = (req, res) => {
    ...
    const ids = agents.list().map((agent) => agent.id)
    return json(res, 200, { sessions: ids })
  }
