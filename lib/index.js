...
export const inject = ['settings', 'systemPrompt', 'agents', 'webServer', 'sessionTitle', 'workspaceRegistry']
...
export function apply(ctx) {
  ...
  const handleSessions = (req, res) => {
    ...
    // Skip archived sessions (the workspace registry's archive set); when the
    // service is absent (non-web hosts), `archived` is undefined and nothing
    // is filtered.
    const archived = ctx.workspaceRegistry?.archivedSessionIds
    const sessions = agents.list()
      .filter((agent) => !archived?.includes(agent.id))
      .map((agent) => {
